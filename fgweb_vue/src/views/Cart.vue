    ...
    <div class="cart-main">
      ...
      <div class="cart-body" id="cartBody">
        ...
        <div class="cart-body-table">
          <div class="item" v-for="(course_info, key) in cart.course_list" :key="course_info.id">
              <div class="item-1">
					<!-- 全选按钮 -->
                  <el-checkbox @change="change_select_course(course_info)" v-model="course_info.selected"></el-checkbox>
              </div>
              <div class="item-2">
              ...
              <div class="item-3">
                  <div class="price" v-if="course_info.discount.price !== undefined">
              ...
              <div class="item-4"><el-icon @click="delete_course(key)" :size="26" class="close"><Close /></el-icon></div>
          </div>
...
const change_select_course = (course_info)=>{
  // 切换商品的勾选状态
  let token = sessionStorage.access_token || localStorage.access_token;
  cart.select(course_info.id, course_info.selected, token);
    ...
    ()=>{
      ...
      if(cart.checked){
        ...
        if(!(cart.selected_course_total === cart.course_list.length)){
          cart.select_all(true, token);
        }

      }

      // 当用户切换为全不选时，判断当前切换时，购物车是不是处于全选状态
      if(cart.checked === false && cart.selected_course_total === cart.course_list.length){
        // 全不选
        // 让客户端的所有课程状态全部都为False
        cart.course_list.forEach((course, key)=>{
          course.selected = false
        })
        cart.select_all(false,token);
      }
    }
)


const delete_course = (key)=>{
  // 从购物车中删除课程
  let token = sessionStorage.access_token || localStorage.access_token;
  cart.delete_course(key, token);
}
