            ...
            <div class="actual-header-wrap">
                ...
                <div class="actual-header-search">
                    <div class="search-inner">
                        ...
                        <img class="actual-search-button" @click="course.search_course" src="../assets/search.svg" />
                    </div>
                    <div class="actual-searchtags">
                    </div>
                    <div class="search-hot">
                        <span>热搜：</span>
                        <a href="" @click.stop.prevent="seaerch_by_hotword(hot_word)" v-for="hot_word in course.hot_word_list" :key="hot_word">{{hot_word}}</a>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="type-wrap">
                <div class="one warp">
                    ...
                    <ul class="items">
                        ...
                        <li :class="{cur: course.current_direction === direction.id}" v-for="direction in course.direction_list" :key="direction.id"><a href="" @click.prevent="course.current_direction=direction.id">{{direction.name}}</a></li>
                    </ul>
                </div>
                <div class="two warp">
                    ...
                    <ul class="items">
                        ...
                        <li :class="{cur: course.current_category === category.id}" v-for="category in course.category_list" :key="category.id"><a href="" @click.prevent="course.current_category=category.id">{{category.name}}</a></li>
                    </ul>
                </div>
            </div>
        </div>
        ...
        <div class="main-wrap">
            <div class="filter clearfix">
                <div class="sort l">
                  ...
                  <a href="" :class="{on:course.ordering==='-price'}" @click.prevent.stop="course.ordering=(course.ordering==='-price'?'':'-price')">价格</a>
                </div>
                <div class="other r clearfix"><a class="course-line l" href="" target="_blank">学习路线</a></div>
            </div>
            <ul class="course-list clearfix">
              <li class="course-card" v-for="course_info in course.course_list" :key="course_info.id">
                <router-link :to="`/project/${course_info.id}`">
                    ...
                    <!-- <p class="one">
                        ...
                        <span class="discount r">
                          ...
                          <i class="countdown" v-if="course_info.discount.expire > 0">{{parseInt(course_info.discount.expire/86400)}}<span class="day">天</span>{{fill0(parseInt(course_info.discount.expire / 3600 % 24))}}:{{fill0(parseInt(course_info.discount.expire/60%60))}}:{{fill0(parseInt(course_info.discount.expire%60))}}</i>
                        </span>
                    </p> -->
...
course.get_course_direction()
// course.get_course_category()

const seaerch_by_hotword = (hot_word)=>{
  // 根据热搜词搜索课程
...
if(route.query.word){
  seaerch_by_hotword(route.query.word)
}else{
  course.get_course_list();
}


// 监听切换不同的学习方向，重新获取对应学习方向下的所有课程分类与课程信息
watch(
...
// 添加商品课程到购物车
const add_course_to_cart = (course_info)=>{
   let token = sessionStorage.access_token || localStorage.access_token;
   cart.add(course_info.id, token).then(response=>{
